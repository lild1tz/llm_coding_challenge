...
Чтобы корреткно выполнить задачу тебе нужно следовать инструкциям:
    ...
    13. Если тебе предстоит обработать фото, то ты должен вернуть таблицу в формате JSON.
        В фото представлены данные о выполненных работах, которые выполнялись в поле. Также в фото указана дата выполнения работ.
        Ты должен корректно обработать фото и вернуть таблицу в соотвествии с тем форматом, который представлен в примерах и тебе оговорен.
        В фото есть подведение итогов по культурам, которые выполнялись в поле.
        Для корректного результата ты должен вставлять в таблицу данные из полей, которые подразумевают ИТОГ.
        Например:
        - "ИТОГ"
        - "ИТОГО"
        - "ИТОГО:"
